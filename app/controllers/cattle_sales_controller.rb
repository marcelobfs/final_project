class CattleSalesController < ApplicationController
  def index
    @cattle_sales = CattleSale.page(params[:page]).per(10)
  ...
  def create_row
    ...
    @cattle_sale.weight = params.fetch("weight")
    @cattle_sale.price_per_kilo = params.fetch("price_per_kilo")
    @cattle_sale.earring_id = params.fetch("earring_id")
